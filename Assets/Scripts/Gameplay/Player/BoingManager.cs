...
namespace KekeDreamLand
{
    ...
    public class BoingManager : MonoBehaviour
    {
        ...
        [SerializeField] private LayerMask whatIsPeckable;
        ...
        public int LifePoints
        {
            ...
            set {
                ...
                // Update HUD.
                GameManager.instance.levelMgr.UpdateLifePoints(lifePoints);
            }
        }
        ...
        private IEnumerator AttackEffect()
        {
            ...
            Vector2 pos = new Vector2(transform.position.x + transform.localScale.x * (attackHitbox.x / 2), transform.position.y + 0.05f);
            Collider2D[] hits = Physics2D.OverlapBoxAll(pos, attackHitbox, 0.0f, whatIsPeckable);

            // Send message to all collider2D which can be pecked inside the hitbox.
            foreach (Collider2D hit in hits)
            {
                if (hit && hit.tag != "Player")
                {
                    hit.gameObject.SendMessage("OnPeck", SendMessageOptions.DontRequireReceiver);
                }
            }
        ...
        private void BounceEffectInRange()
        {
            ...
            Collider2D[] mobs = Physics2D.OverlapCircleAll(transform.position, bouncingEffectRadius, whatIsPeckable);
        ...
        private IEnumerator Invulnerability()
        {
            ...
            if (!GameManager.instance.levelMgr.IsInternalTransition)
